...
public class cpuSideCode : MonoBehaviour {
	...
	static public ComputeBuffer timeBuffer;			
	...
	static void initTexture(){
		...
		outputTexture.filterMode = FilterMode.Bilinear;			// not necessary, I just wanted to have clean pixels
	}
	static public void initCanvas(){
		...
		mainCanvas.GetComponent<UnityEngine.UI.CanvasScaler>().matchWidthOrHeight = 0f;
		...
		outputImage.GetComponent<RectTransform>().sizeDelta = new Vector2(2400, 2400);
	}
	...
	void Update(){
		...
		//Vector3 rotation = new Vector3(0,0,Time.deltaTime/2);
		//outputImage.rectTransform.Rotate(rotation);
	...
	void input(){					// in this method we simply change size and position of the view rect in fractal dimension, but it is always projected to the texture
		...
		depthFactor += 0.4 * depthFactor * Time.deltaTime*2/(1+Mathf.Sqrt(Time.time));
		inputChange = true;
		//}
