...
{        	
    ...
    private void FixedUpdate()
	{
		...
		//m_RemainingMovement -= distanceTravelled;

		//print("m_RemainingMovement after removing distancetravelled: "+m_RemainingMovement);

		if(initialVel.magnitude>0)
		{
			//m_RemainingVelM = ((m_Vel.magnitude*Time.fixedDeltaTime)-distanceTravelled.magnitude)/(m_Vel.magnitude*Time.fixedDeltaTime);
			//print("m_RemainingVelM = ((m_Vel.magnitude*Time.fixedDeltaTime)-distanceTravelled.magnitude)/(m_Vel.magnitude*Time.fixedDeltaTime)");
			//print("m_RemainingVelM = (("+m_Vel.magnitude*Time.fixedDeltaTime+")-"+distanceTravelled.magnitude+")/("+m_Vel.magnitude*Time.fixedDeltaTime+")");
			...
			//print("m_RemainingVelM = (((initialVel.magnitude*Time.fixedDeltaTime)-distanceTravelled.magnitude)/(initialVel.magnitude*Time.fixedDeltaTime))");
			//print("m_RemainingVelM = ((("+initialVel.magnitude*Time.fixedDeltaTime+")-"+distanceTravelled.magnitude+")/("+initialVel.magnitude*Time.fixedDeltaTime+"))");
			//print("m_RemainingVelM = ("+m_RemainingMovement.magnitude+"/"+initialVel.magnitude*Time.fixedDeltaTime+")");
		...
		//int contactPoints = 0;
		//if(m_LeftWalled){contactPoints++;}
		//if(m_RightWalled){contactPoints++;}
		//if(m_Ceilinged){contactPoints++;}
		//if(m_Grounded){contactPoints++;}
		...
		if(m_LeftWalled)
		{
			if(m_Grounded)
			{
				...
				if(m_Vel.x >= 0)
				{
					float leftSteepness = Vector2.Angle(Vector2.right, Perp(m_LeftNormal));
					//print("LS="+leftSteepness);
					float groundSteepness = Vector2.Angle(Vector2.right, Perp(m_GroundNormal));
			...
			else if(m_Ceilinged)
			{
				...
				if(m_Vel.y > 0)
				{
					...
					float leftSteepness = Vector2.Angle(Vector2.left, Perp(m_LeftNormal));
					print("LS="+leftSteepness);
					float ceilingSteepness = Vector2.Angle(Vector2.left, Perp(m_CeilingNormal));
		...
		if(m_RightWalled)
		{
			if(m_Grounded)
			{
				if(m_Vel.x > 0) //If moving right, use the steepest angle colliding surface.
				{
					float rightSteepness = Vector2.Angle(Vector2.right, Perp(m_RightNormal));
				...
				else if(m_Vel.x <= 0) //If moving left or down, use the shallowest angle colliding surface.
				{
					float rightSteepness = Vector2.Angle(Vector2.right, Perp(m_RightNormal));
					print("RS="+rightSteepness);
					float groundSteepness = Vector2.Angle(Vector2.right, Perp(m_GroundNormal));
			...
			else if(m_Ceilinged)
			{
				...
				{
					print("ASCENDING IMPACT");
					float rightSteepness = Vector2.Angle(Vector2.left, Perp(m_RightNormal));
					print("RS="+rightSteepness);
					float ceilingSteepness = Vector2.Angle(Vector2.left, Perp(m_CeilingNormal));
					print("CS="+ceilingSteepness);
		...
		if (m_Grounded&&!m_RightWalled&&!m_LeftWalled) //Handles velocity along ground surface.
		{
			...
			if(m_Ceilinged)
			{
				if(m_Vel.y > 0) //If moving up, use the ceiling surface
				{
					DirectionChange(m_CeilingNormal);
				}
				else if(m_Vel.y <= 0) //If moving down or horiz, use the ground colliding surface.
				{
					DirectionChange(m_GroundNormal);
				}
			}
