...
//		float dynamicScaleX = (startScaleX/(1+layerDepth*distanceSizeRatio));
//		float dynamicScaleY = (startScaleY/(1+layerDepth*distanceSizeRatio));
//		o_CloudLayer[layerDepth-1].transform.localScale = new Vector3(dynamicScaleX,dynamicScaleY,1);
//		o_CloudLayer[layerDepth-1].layer = 11;
//		ParallaxLayer prlx = o_CloudLayer[layerDepth-1].GetComponent<ParallaxLayer>();
//		float para = (layerDepth)/(layerDepth+distanceParallaxRatio);
//		//print("PARA: "+para+", layerDepth: "+layerDepth);
//		//float parallaxM = 0.5f*para;
//		prlx.speedX = para;
//		prlx.speedY = para;
...
//		Vector2 cloudOrigin = new Vector2(0, cloudLayerElevation);
//		GameObject newCloudLayer = (GameObject)Instantiate(p_CloudLayer, cloudOrigin, Quaternion.identity);
//		float dynamicScaleX = (startScaleX/(layerDepth*distanceSizeRatio));
//		float dynamicScaleY = (startScaleY/(layerDepth*distanceSizeRatio));
//		newCloudLayer.transform.localScale = new Vector3(dynamicScaleX,dynamicScaleY,1);
...
//	}
	...
	GameObject SetupCloudLayer(int layerDepth)
	{
		...
		//prlx.distanceKM = ((float)layerDepth/(float)layerDensity)*zDistanceKM;
		prlx.distanceKM = (float)layerDepth*(float)layerDepth*distributionCurveM;
		print("((float)layerDepth/(float)layerDensity)*zDistanceKM="+(((float)layerDepth/(float)layerDensity)*zDistanceKM));
		//prlx.speedY = para;
		SpriteRenderer[] cloudRenderer = newCloudLayer.GetComponentsInChildren<SpriteRenderer>();
